...
### 2.1 Tech stack analysis and development (via context7 MCP)

Use **context7 MCP tools** to discover existing patterns, dependencies, and architecture in the tech stack or if new project to plan out full tech stack frameworks:

Document:
- Reusable modules, services, and helpers  
- File paths and structure conventions  
- Naming, typing, and testing standards  
- Any architectural constraints  

### 2.2 Web Research (if applicable)
...
### 2.3  Knowledge Base Search (if instructed)

If Archon RAG is available and relevant:
- Use `mcp_archon__rag_get_available_sources()` to see available documentation
- Search for relevant patterns: `mcp_archon__rag_search_knowledge_base(query="...")`
...
### 2.4 Codebase Analysis (for existing projects)
...
### 2.5 Sequential Reasoning & Decomposition

Using SequentialThinking MCP validate all content using the following method:

1. **Analyze** input requirements and existing patterns.  
2. **Decompose** into minimal sequential subtasks.  
3. **Execute** each subtask in order.  
4. **Validate** outputs of each stage using the gates below.  
